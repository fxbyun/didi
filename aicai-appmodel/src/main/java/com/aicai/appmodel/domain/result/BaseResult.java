...
import com.aicai.appmodel.internationalization.InternationalizationMsgManager;
import com.aicai.appmodel.internationalization.RequestLocaleThreadLocalHolder;
...
public class BaseResult implements Serializable {
	...
	public boolean isSuccess() {
		...
		if (!isEmpty) {
			Locale requestLocale = RequestLocaleThreadLocalHolder.getAcceptLanguateLocale();
			if (requestLocale != null) {
				try {
					for (Map.Entry<String, String> entry : errorList.entrySet()) {
						String msg = InternationalizationMsgManager.getMessage(entry.getKey(), requestLocale);
						if (msg != null) {
							entry.setValue(msg);
						}
					}
				} catch (Throwable ex) {
					log.error("{requestLocale:\"" + requestLocale + "\"}", ex);
				}
			}
		}
		return isEmpty;
	...
	/**
	 * 对于有多语言需求的前端,调用isSuccess后,需再调用此方法
	 * 
	 * @param local
	 */
	public void internalizeationErrorMsg(Locale requestLocale) {
		boolean isEmpty = errorList.isEmpty();
		if (!isEmpty) {
			for (Map.Entry<String, String> entry : errorList.entrySet()) {
				String msg = InternationalizationMsgManager.getMessage(entry.getKey(), requestLocale);
	...
	/**
	 * 消息本地化可看 章节“消息本地化” http://wiki.inzwc.com/tech/wiki/index.php/AicaiAppModel#.E6.B6.88.E6.81.AF.E6.9C.AC.E5.9C.B0.E5.8C.96
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param errorCode
	 *            定义规范 errorCode 英文，不超过50长度，以“点”分隔，：“粗模块.细模块.错误意思"
